...
**Example 9.1.1. Coin Toss.** Considering the simple case of coin tossing, if we flip a fair coin many times, we expect to see heads about 50\% of the time. If we flip the coin only a few times, however, we could see a different distribution just by chance. Indeed, there is a non-zero probability of observing all heads (and this even if the sample is very large). Figure \@ref(fig:ChBayesHistogram) illustrates this very fact by showing the number of heads observed in 100 samples of five iid tosses; in this specific example, we observe six samples for which all tosses are heads.[^ChBayesFt1]
...
(ref:ChBayesBayesianToss) **Posterior probability density function of parameter $q$ for a sample of five data points**
...
[^ChBayesFt4]: There is also a rich history blending prior information with data in loss modeling and in actuarial science, generally speaking; it is known as credibility. For more details on experience rating using credibility theory, see Chapter \@ref(ChapCredibility).
...
Despite these disadvantages, the Bayesian approach remain powerful and flexible for many actuarial problems. 
...
**Example 9.1.3. Actuarial Exam Question.**
...
**Example 9.1.4. Singapore Insurance Data.**
...
**Example 9.2.3. Actuarial Exam Question.**
You are given:
...
Using the first 250 lines of the Singapore insurance dataset (see Example 9.1.4 for more details on this dataset), find the two posterior distributions as well as the posterior expected value of the probability $q$ under both prior assumptions. 
...
**Proposition 9.3.1. Beta--Binomial Conjugate Family.**
...
**Example 9.3.1. Actuarial Exam Question.**
You are given:
...
**Example 9.3.2. Impact of Beta Prior on Posterior.**
...
Using again the first 250 lines of the Singapore insurance dataset (see Example 9.1.4 for more details on this dataset), find the two posterior distributions.
...
**Proposition 9.3.2. Gamma--Poisson Conjugate Family.**
...
**Example 9.3.3. Actuarial Exam Question.**
You are given:
...
**Solution.**
Using the results of Proposition 9.3.3, we can obtain the following posterior distributions:
...
Interestingly, as shown in Example 9.3.4, the prior distribution can have some impact on the final posterior distribution. When the prior assumption about the mean is very precise, having a few data points do not create a huge gap between the prior and the posterior (see the light gray curves in Figures \@ref(fig:ChBayesNormalPrior1) and \@ref(fig:ChBayesNormalPosterior2)). When the prior is very imprecise, on the other hand, then the data are allow to speak, and the posterior can be quite different from the prior distribution.
...
A common way to generate draws of the parameter posterior distribution is to create Markov chains for which their stationary distributions correspond to the posterior of interest. These Markov chain-based methods are known as Markov chain Monte Carlo (MCMC) methods in the literature. This section provides a brief overview of these methods and of their uses. We do not intend to give much of the theory behind these methods, which would require a deep understanding of Markov chains and their theory.[^ChBayesFt10] Instead, we focus on their applications in insurance and loss modeling. Specifically, in the next two subsections, we introduce the two most common MCMC methods; that is, the `r Gloss('Gibbs sampler')` of @gelfand1990sampling and the `r Gloss('Metropolis--Hastings algorithm')` of @hastings1970monte and @metropolis1953equation.
...
In general, the coefficients $\alpha$ and $\beta$ are unknown and need to be estimated. The experimenter can rely on Bayesian inference to find out the posterior distribution of the parameters $\alpha$ and $\beta$ along with that of $\sigma^2$. For the rest of the subsection, we investigate a specific application of Gibbs sampling to the context of linear regression. 
...
**Proposition 9.4.1. Full Conditional Distributions of Bayesian Linear Regression Parameters.**
...
**Example 9.4.1. Bayesian Linear Regression.**
...
Once we have the posterior parameter samples, we can get multiple quantities of interest. For instance, the posterior mean of parameters $\alpha$, $\beta$, and $\sigma^2$ are $9.843$, $-0.0208$, and $2.551$, respectively.
...
Then, we can repeat the same process for all other parameters to obtain $\theta_2^{(1)}$ to $\theta_k^{(1)}$, while replacing the parameters  $\pmb{\theta}_{\backslash i}$ by their most current values in the chain. Once we have updated all values, we can repeat this process for all $m$ in $\{2,3,...,M\}$, similar to the iterative process used in the Gibbs sampler. 
...
**Example 9.4.4. Markov Chain Diagnostics.**
Consider the setup of Example 9.4.2. Using chains of 51,000 iterations and a burn-in of 1,000 iterations, calculate the various Markov chain diagnostics mentioned above.
...
## Bayesian Inference in Practice {#ChBayes:SecPractice}

------------------------------------------------------------------------

In Section \@ref(ChBayes:SecPractice), you learn how to:

- Describe the main computing resources available for Bayesian inference and modeling.
...
**Example 9.5.1. The `nimble` package.**
Similar to the setup of Example 9.4.2, consider that each policyholder's claim count (frequency) is distributed as a Poisson random variable such that
$$
p_{N_i\,|\,\Lambda=\lambda}(n_i) = \frac{\lambda^{n_i}e^{-\lambda}}{n_i!},
$$
where $n_i$ is the number of claims associated with the $i^{\text{th}}$ policyholder. Unlike the previous example, however, let us assume an inverse gamma prior with a shape parameter of 2 and a scale parameter of 5.[^ChBayesFt11]

[^ChBayesFt11]: Note that the inverse gamma prior combined with a Poisson distribution does not generally lead to closed-form posterior densities and thus requires us to use MCMC methods.
...
```{r ChBayesNIMBLE1, echo = TRUE, eval = TRUE}
...
```{r ChBayesNIMBLE2, echo = TRUE, eval = TRUE}
...
```{r ChBayesNIMBLE3, echo = TRUE, eval = FALSE}
...
(ref:ChBayesPractice) **Trace plot and posterior density for parameter $\lambda$**

```{r ChBayesPractice, echo = FALSE, fig.cap='(ref:ChBayesPractice)', out.width = '80%', fig.asp = 0.75, fig.align = 'center'}
