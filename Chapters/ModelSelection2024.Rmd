...
Section \@ref(S:MS:NonParEst) introduced nonparametric estimators in
...
better visualize the data. The Chapter 6 *Technical Supplement A.1*
...
**Example 6.1.1. Actuarial Exam Question.** The graph below shows a $pp$
...
#### Pearson's chi-square test {#S:Pearsons}
...
To illustrate application of the Pearson's chi-square test, we use the example introduced in Section \@ref(S:freq-real-example): In $1993$, a portfolio of $n=7,483$ automobile insurance policies from a major Singaporean insurance company had the distribution of auto accidents per policyholder as given in [Table 6.2].
...
\]
Now if we use Poisson ($\hat{\lambda}_{MLE}$) as the fitted distribution, then a tabular comparison of the fitted counts and observed counts is given by [Table 6.3] below, where $\hat{p}_k$ represents the estimated probabilities under the fitted Poisson distribution. 
...
parametric form, as summarized in Section \@ref(S:MS:NonParEst).
...
For selecting the appropriate distribution, statistics that compare a parametric fit to a nonparametric alternative, summarized in Section \@ref(S:MS:Tools:Stats), are useful for model comparison. For discrete
...
Following Example 2.3.1, we use simulated data in Examples 6.4.2 through 6.4.4 to compare the AIC
...
**Example 6.4.2. In-sample AIC and out-of-sample MSE for normal
...
**Example 6.4.3. MSE and MAE for right-skewed outcomes - lognormal
...
**Example 6.5.1. Cross-validation in right-skewed outcomes - lognormal
...
**Example 6.5.2. Cross-validation in right-skewed outcomes - gamma
...
8.3.1 in Chapter \@ref(ChapSimulation) uses the Wisconsin Property Fund to
...
functions in Section \@ref(S:MS:ModifiedData1). For model selection, the
tools from Section \@ref(S:MS:ToolsModelSelection) can be extended to cases
of modified data.
...
Figure \@ref(fig:qqCensored), the lognormal distribution seems
to fit the censored data much better those based on the normal and
exponential distributions.

(ref:qqCensored) **Quantile-Quantile ($qq$) Plots for Bodily Injury Claims. The horizontal axis gives the empirical quantiles at each observation. The vertical axis gives the quantiles from the fitted distributions; lognormal quantiles are in the left-hand panel, normal quantiles are in the middle, and exponential in the left-hand panel.**


```{r qqCensored, fig.cap='(ref:qqCensored)', echo = FALSE, warning=FALSE, out.width='150%', fig.asp=0.40}
library(GofCens)
library(grid)
library(gridExtra)
...
plot1 <- grid.grabExpr(GofCens::probPlot(BIData$AmountPaid, cens = UnCensored, distr = "lognormal", plots = "QQ", ggp=TRUE,prnt = FALSE, mtitle=F, main=""))
## qq-plot for normal distribution
plot2 <- grid.grabExpr(GofCens::probPlot(BIData$AmountPaid, cens = UnCensored, distr = "normal", plots = "QQ", ggp=TRUE,prnt = FALSE, mtitle=F, main=""))
## qq-plot for exponential distribution
plot3 <- grid.grabExpr(GofCens::probPlot(BIData$AmountPaid, cens = UnCensored, distr = "exponential", plots = "QQ", ggp=TRUE,prnt = FALSE, mtitle=F, main=""))
...
```{r ref.label = 'qqCensored', eval = FALSE, echo = SHOW_PDF}
```
...
**Example 6.6.2. Bodily Injury Claims and Goodness of Fit Stastistics.** For the Boston auto bodily injury claims with right-censoring, we may use the goodness of fit statistics to evaluate the fitted lognormal, normal and exponential
distributions. For the Kolmogorov-Smirnov, Cramer-von
...
```{r ref.label = 'KsCensored', eval = FALSE, echo = SHOW_PDF}
```
...
-   **Edward W. (Jed) Frees** and **Lisa Gao**, University of Wisconsin-Madison, are the principal authors of the initial version of this chapter. Email:[jfrees\@bus.wisc.edu](mailto:jfrees@bus.wisc.edu){.email} for
