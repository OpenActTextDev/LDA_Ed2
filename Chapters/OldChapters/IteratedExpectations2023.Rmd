...
As described in Section \@ref(ChBayes:SecConjugate), for conjugate distributions the posterior and the prior come from the same family of distributions. In insurance applications, this broadly occurs in a "family of distribution families" known as the linear exponential family which we introduce first.
...
The dependent variable $x$ may be discrete, continuous or a hybrid combination of the two. Thus, $f\left( \cdot\right)$ may be interpreted to be a density or mass function, depending on the application. [Table 18.1] provides several examples, including the normal, binomial and Poisson distributions.
