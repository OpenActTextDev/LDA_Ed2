...
**Solution.** From the probability density functions expression in Section \@ref(S:ContinuousDistributions), the $k$th raw moment is
...
Because all the positive moments exist, we have $k^{\ast}=\infty$. Thus, in accordance with the moment-based classification method in Definition  `r chapnum`.1, the gamma distribution is light-tailed.  
...
Let $X\sim Weibull(\theta,\tau)$, with $\theta>0$ and $\tau>0$. Show that $\mu_k' < \infty$ for all $k>0$.
...
*  3.  When we need to compare the tail weight between two light-tailed distributions (where both have all finite positive moments), the moment-based method is no longer informative (see, e.g., Examples `r chapnum`.2.1 and `r chapnum`.2.2).
...
Here, $inf$ is the infimum operator so that the $VaR$ measure outputs the smallest value of $x$ such that the associated `r Gloss('cdf')`  exceeds or equates to $q$. This is simply the `r Gloss('quantile')` that was introduced in Section \@ref(S:LS:Quantiles) and further developed in Section \@ref(S:MS:QuantileEstimator).
...
* relatively simple to compute for many distributions with closed-form distribution functions, and
...
Consider an insurance loss random variable $X$ with a lognormal distribution having parameters $\mu \in \mathbf{R}$ and $\sigma>0$. Show that
...
We have so far checked that $\mathrm{E}[\cdot]$ is a coherent risk measure and that $\mathrm{SD}(\cdot)$ is not. [Exercise 13.1] asks you to study the coherent property for the standard deviation principle \@ref(eq:SD-principle) which is a linear combination of coherent and incoherent risk measures. 
...
Recall from Section \@ref(S:Chap5Reinsurance) that `r Gloss('reinsurance', '1.1, 10.4')` is simply insurance purchased by an insurer. Insurance purchased by non-insurers is sometimes known as  `r Gloss('primary insurance')` to distinguish it from reinsurance. Reinsurance differs from personal insurance purchased by individuals, such as auto and homeowners insurance, in contract flexibility. Like insurance purchased by major corporations, reinsurance programs are generally tailored more closely to the buyer. For contrast, in personal insurance buyers typically cannot negotiate on the contract terms although they may have a variety of different options (contracts) from which to choose.
...
[Example 13.4.6]: ../docs/ChapPortMgt.html#Ex:134.6>
...
[Exercise 13.1]: ../docs/ChapPortMgt.html#Exer:13.1>

**Exercise 13.1.** In this exercise, you will demonstrate that only under specific circumstances can the standard deviation principle \@ref(eq:SD-principle) be considered a coherent risk measure.
...
```{r echo=SHOW_PDF, ref.label = 'Exercise132', eval = FALSE}
```
...
**Exercise 13.3. Risk Measures with Stop-Loss**. Consider the stop-loss arrangement with retention level $M$ described in Section \@ref(S:NonProportionalRe).
...
**a.** The distribution function for the limited random variable $X \wedge M$ is
...
**b.** From equation \@ref(eq:ESExpressions), the expected shortfall for retained risks can be expressed as
...
```{r echo=SHOW_PDF, ref.label = 'Exercise133', eval = FALSE}
```
