...
### Moments and Moment Generating Functions {#S:Chap3Moments}
...
## Methods of Creating New Distributions {#S:MethodsCreation}
...
### Mixture Distributions for Severity {#S:MixtureSeverity}
...
[Example 4.3.5]: ../docs/ChapSeverity.html#Ex:43.5
...
**Example 4.4.3. Toy Data Set: Continued.**
...
[Table 4.1]: ../docs/ChapSeverity.html#tab:4.1
...
### Parametric Estimation {#S:Parametric}

Section \@ref(S:ContinuousDistn) has focused on parametric distributions that are commonly used in insurance applications. However, to be useful in applied work, these distributions must use "realistic" values for the parameters. In this section we cover three methods for estimating parameters: Method of moments, Percentile matching, and Maximum likelihood estimation. 
...
**Example 4.4.10. Actuarial Exam Question.**
...
[Example 4.4.12]: ../docs/ChapSeverity.html#Ex:44.12

**Example 4.4.12. Wisconsin Property Fund.** To see how maximum likelihood estimators work with real data, we return to the 2010 claims data introduced in Section \@ref(S:LGPIF). 
...
Results from the fitting exercise are summarized in Figure \@ref(fig:MLEComparea). Here, the black “longdash” curve is a density estimator of the actual data (introduced in Section \@ref(S:MS:Density)); the other curves are parametric curves where the parameters are computed via maximum likelihood. We see poor fits in the red dashed line from the exponential distribution fit and the blue dotted line from the gamma distribution fit. Fits of the other curves, Pareto, lognormal, and GB2, all seem to provide reasonably good fits to the actual data. Chapter \@ref(ChapModelSelection) describes in more detail the principles of model selection. 
...
## Exercises with a Practical Focus  {#S:LossExercises}
...
One can fit a distribution to the losses. An analysis, summarized in Figure \@ref(fig:GPALossFit43), shows the results from fitting  via maximum likelihood the gamma, Pareto, and lognormal distributions to incurred losses. This figure suggests that the lognormal distribution appears to be the best fit.
...
```{r GPALossFit43, fig.cap='**Distribution of Group Personal Accident Losses with Superimposed Fitted Distributions**', out.width='80%', fig.asp=.6, echo = FALSE}
...
```{r echo=SHOW_PDF, ref.label = 'GPALossFit43', eval = FALSE}
...
- **Zeinab Amin**, The American University in Cairo, is the principal author of the initial version and also the second edition of this chapter. Email: zeinabha@aucegypt.edu for chapter comments and suggested improvements.
