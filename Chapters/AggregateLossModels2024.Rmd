...
The rest of the chapter is structured as follows: Section \@ref(S:AggLoss:IndRiskModel) and Section \@ref(S:AggLoss:CRM) provide details on the individual risk model and collective risk model respectively. Section \@ref(S:AggLoss:Compute) presents methods for computing the distribution of aggregate claims. Section \@ref(S:AggLoss:CovMod) discusses the effect of coverage modifications on the aggregate losses. Technical materials are summarized in Section \@ref(AL-further-reading-and-resources).


```{r child = './Quizzes/Quiz51.html', eval = QUIZ}
```

## Individual Risk Model {#S:AggLoss:IndRiskModel}
...
### Aggregate Loss Distribution {#S:AggLoss:AggLossDist}
...
## Computing the Aggregate Claims Distribution {#S:AggLoss:Compute}
...
1. Individual Risk Model: $S_n=X_1+\cdots+X_n$    
    ...
    + Generate each individual loss realization $x_{ij}$ for $i=1,\ldots,n$. For example, this can be done using the inverse transformation method (Section \@ref(S:InverseTransform)).
...
[Example 7.4.3]: ../docs/ChapAggLossModels.html#Ex:74.3

**Example `r chapnum`.4.3.**
Recall Example `r chapnum`.3.6 with an individual's claim frequency $N$ has a Poisson distribution with mean $\lambda=25$ and claim severity $X$ is uniformly distributed on the interval $(5,95)$. Using a simulated sample of 10,000 observations, estimate the mean and variance of the aggregate loss $S_N$. In addition, use the simulated sample to estimate the probability that aggregate claims for this individual will exceed 2,000 and compare with the normal approximation estimates from Example `r chapnum`.3.6.
...
## Effects of Coverage Modifications {#S:AggLoss:CovMod}
...
### TS `r chapnum`.A.1. Individual Risk Model Properties {#S:AggLoss:TSA1}
...
### TS `r chapnum`.A.2. Relationship Between Probability Generating Functions of $X_i$ and $X_i^T$ {#S:AggLoss:TSA2}
...
### TS `r chapnum`.A.3. Moment Generating Function of Aggregate Loss $S_N$ in Example `r chapnum`.3.8 {#S:AggLoss:TSA2}
