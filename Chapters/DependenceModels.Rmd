...
```{r IntroPlot, fig.cap='(ref:IntroPlot)',echo=FALSE, warning=FALSE, out.width='80%', fig.asp=.75, fig.align='center',message=FALSE}
library(ggplot2)
#Insample <- read.csv("/Users/okinean/Dropbox/Phd/Frees/DependenceModelingChapter/Book2edition/Loss-Data-Analytics-Ed1-master/Data/Insample.csv", header=T,na.strings=c("."), stringsAsFactors=FALSE)
...
We would like to know whether the distribution of ${\tt LOSS}$ depends on the distribution of ${\tt ALAE}$ or whether they are statistically independent. To visualize the relationship between losses and expenses, the scatterplots in Figure \@ref(fig:Scatter) are created on dollar and log dollar scales. It is difficult to see any relationship between the two variables in the left-hand panel. Their dependence is more evident when viewed on the log scale, as in the right-hand panel. This sections elaborates basic measures for modeling the dependence between variables. 

(ref:Scatter) **Scatter Plot of LOSS and ALAE**

```{r  Scatter, message=FALSE, warning=FALSE, echo=FALSE, out.width='80%', fig.align='center',fig.cap='(ref:Scatter)', fig.height=5.5, fig.width=9}
library(copula)
...
You can obtain the `r Gloss('Pearson correlation')` statistic $r$ using the `cor()` function in `R` and selecting the `pearson` method. This is demonstrated below by using the ${\tt LOSS}$ rating variable in millions of dollars and ${\tt ALAE}$ amount variable in dollars from the dataset in Figure \@ref(fig:Scatter). 
...
**Example `r chapnum`.1.1. Calculation by Hand.**

You are given the following six observations:
...
The Spearman correlation is simply the product-moment correlation computed on the ranks:
...
\hline \textbf{Observation} & x \textbf{ value} & y  \textbf{ value} &\textbf{Rank of} & \textbf{Rank of}  & (R(X)_i - \bar{R(X)})& (R(Y)_i - \bar{R(Y)}) &(R(X)_i - \bar{R(X)})(R(Y)_i - \bar{R(Y)}) \\
   & &  & x_i\quad (R(x)) & y_i  \quad (R(Y)) & & &\\
...
**Example `r chapnum`.1.2. Calculation by Hand.**
...
We can obtain the Kendall's tau using:
...
### Tail Dependence Coefficients {#Sec:tdc}
...
In Figure \@ref(fig:tdcplot1), the concept of tail dependence is demonstrated through an example. The figure showcases two randomly generated variables with a Kendall's Tau of 0.7. On the left side of Figure \@ref(fig:tdcplot1), the variables ($X$ and $Y$) are simulated using the bivariate normal distribution, while on the right side, they are generated using the bivariate t-distribution. Although both sides display a Kendall's Tau of 0.7, there is a difference in the upper right quadrant (above the dashed lines) on each panel. In the left panel, the values in the upper right quadrant (upper tails of $X$ and $Y$) are independent, while in the right panel, the upper tail values appear to be correlated (the upper right corners of the right panel contain 4 points). This suggests that the probability of $Y$ occurring above a high threshold (e.g., the dashed line in the figure) when $X$ exceeds the same threshold is higher in the right panel than in the left panel of Figure \@ref(fig:tdcplot1).



(ref:tdcplot1) **Left Panel: Upper tails of $X$ and $Y$ are independent. Right Panel: Upper tails of $X$ and $Y$ appear to be dependent.**

```{r tdcplot1, message=FALSE, echo=FALSE, warning=FALSE,out.width='90%',fig.align='center',fig.cap='(ref:tdcplot1)'}
...
$$
\lambda_{\text{lo}} {=} \lim _{u \rightarrow 0^+} \mathrm{Pr}\left\{X \leq F_X^{-1}(u) \mid Y \leq Y^{-1}(u)\right\} .
$$
...
Figure \@ref(fig:ScatterR) shows the scatter plot of the ranks of the ${\tt LOSS}$ variable and the ${\tt ALAE}$ variable. You can obtain the upper and lower tail dependent coefficient using the `tdc()` function from the `FRAPO` package in `R`. From below, $\hat{\lambda}_{\text{up}}=0.39$, at $k=75$ (note that $n=1500$), between the ${\tt LOSS}$ variable and the ${\tt ALAE}$ variable and $\hat{\lambda}_{\text{lo}}=0.13$. The results implies the losses and expenses variables appear to be more upper-tailed dependent. 

(ref:ScatterR) **Scatter Plot of Ranks of LOSS and ALAE**

```{r ScatterR, message=FALSE, echo=FALSE, warning=FALSE,fig.height=3.9, fig.width=3.9,fig.align='center',fig.cap='(ref:ScatterR)'}
## Claim , Closed
#set.seed(10)
#summary(rank(Claim))
...
Copulas are widely used in insurance and many other fields to model the dependence among multivariate outcomes as they expresses the dependence between the variables explicitly. Recall the joint cumulative distribution function ($cdf$) for two variables $Y_1$ and $Y_2$ is given by: 

$$
{F}(y_1, y_2)= {\Pr}(Y_1 \leq y_1, Y_2 \leq y_2)
$$

For the the multivariate case in $p$ dimensions, we have:
...
exists. For discrete case, readers can see @joe2014dependence and @genest2007methods. For the bivariate case where $p=2$, we can write a copula and the distribution function of two random variables as 
...
In Figure \@ref(fig:IndCop), both the distribution function and scatter plot of observations generated from the independence copula are displayed. The scatter plot indicates that there is no correlation between the two components, $U_1$ and $U_2$.


(ref:IndCop) Left: Scatterplot of observations from Independence Copula. Right: Plot for distribution function for Independence Copula.

```{r IndCop, message=FALSE, warning=FALSE, comment="", echo=FALSE, fig.align='center', fig.height=4, fig.width=8, fig.cap='(ref:IndCop)'}
par(mfrow=c(1,2))
d<-2
...
persp(ic, pCopula, zlab="C(u,v)", theta=50, cex.lab=0.9, cex.axis = 0.5)
...
```{r, echo=SHOW_PDF, ref.label = 'IndCop', eval=FALSE}
```
...
In Figure \@ref(fig:FrankCop1), we can see scatterplots of data generated from the Frank's copula. As $\gamma$ value changes, we observe that components $U_1$ and $U_2$ become positively or negatively dependent. When $\theta$ approaches 0, \@ref(eq:FrankCopula)) transforms into an independence copula. Also,  Figure \@ref(fig:FrankCop2) provides the distribution and density functions for Frank's copula when $\gamma=12$. In Section \@ref(S:CopTyp), we will explore copula functions other than the commonly used Frank's copula.

(ref:FrankCop1) **Scatterplot of Observations from Frank's Copula**. $\gamma=12$ (left), $\gamma=0$ (middle) and $\gamma=-12$ (right). 

```{r FrankCop1, message=FALSE, warning=FALSE, comment="", echo=FALSE, fig.height=3.2, ,fig.align='center', fig.cap='(ref:FrankCop1)'}
par(mfrow=c(1,3))
d<-2
gamma_par<-12
...
(ref:FrankCop2) Left: Plot for distribution function for Frank's Copula ($\gamma=12$). Right: Plot for the density function for Frank's Copula ($\gamma=12$).

```{r FrankCop2, message=FALSE, warning=FALSE, comment="", echo=FALSE, fig.align='center', fig.cap='(ref:FrankCop2)'}
par(mfrow=c(1,2))
d<-2
gamma_par<-12
fc<-frankCopula(gamma_par, dim=d)
persp(fc, pCopula, zlab="C(u,v)", theta=50, cex.lab=0.9, cex.axis = 0.5)
persp(fc, dCopula, zlab="c(u,v)", theta=50, cex.lab=0.9, cex.axis = 0.5)
```
...
**Example `r chapnum`.2.1. Copula Representation Example**
...
Denote the marginal distribution functions of $X$ and $Y$ as $F_X(x)$ and $F_Y(y)$, respectively. Since $F_{X,Y}(7.2,4.1)=C[F_X(7.2),F_Y(4.1)]$, we can use the marginal distribution functions to obtain the arguments of the copula function:
...
Suppose we have $y_1, \ldots, y_p \sim F$, $U\sim \text{U}(0,1)$ and assume $y_1, \ldots, y_p$ is continuous, which implies $F_1^{-1}(U_i)=Y_i$. Then, the random variables have a multivariate distribution function $C$, given by: 
...
This section analyzes the insurance losses and expenses data with the statistical program `R`. The data set is visualized in Figure \@ref(fig:Scatter). The model fitting process is started by marginal modeling of each of the two variables, ${\tt LOSS}$ and ${\tt ALAE}$. Then we model the joint distribution of these marginal outcomes.
...
The marginal distributions of losses and expenses are fit using the method of maximum likelihood. Specifically, we use the `vglm` function from the `R VGAM` package. Firstly, we fit the marginal distribution of ${\tt ALAE}$. Parameters are summarized in [Table 16.6].
...
### Probability Integral Transformation {#S:PIT}
...
For $p=2$, the Clayton copula is defined by $\gamma \in [-1,\infty)$ is defined by
...
Let's assign the Fr&eacute;chet-H&ouml;effding lower bound as $W$ and the upper bound as $M$. That is, $W=\max(u_1+\cdots+u_p-p+1, 0)$ and $M=\min(u_1,\ldots,u_p)$. It's important to note that $W$ is a copula only if $p=2$, while $M$ is a copula for all $p\geq2$. In dimension two, $W=\max(u_1+u_2-1, 0)$ is known as the **Counter-monotone copula**. It captures the inverse relationship between two variables, that is, two random variables that are perfectly negatively related. On the other hand, $M=\min(u_1,u_2)$ in dimension two is known as the **Co-monotone copula**. It captures the relationship between two variables where one is related to the other by a strictly increasing function, that is, two random variables that are perfectly positively dependent. The co-monotonic copulas can be extended to the multivariate case. However, it's not possible to extend the counter-monotonic copula because it's not possible to have three or more variables where each pair has a direct inverse relationship.


**Example `r chapnum`.5.1. Largest Possible Value Example**
...
Let $C(u,v)$ is the copula that links $X$ and $Y$. Denote the marginal distribution functions of $X$ and $Y$ as $F_X(x)$ and $F_Y(y)$, respectively. Since $F_{X,Y}(7.2,4.1)=C[F_X(7.2),F_Y(4.1)]$, we can use the marginal distribution functions to obtain the arguments of the copula function:
...
**Example `r chapnum`.5.2. Lower Tail Dependence Coefficient Example**
...
## Importance of Dependence Modeling {#S:CopImp}
...
-  **Edward W. (Jed) Frees** and **Nii-Armah Okine**, University of Wisconsin-Madison, and **Emine Selin Sarıdaş**, Mimar Sinan University, are the principal authors of the initial version of this chapter. 
